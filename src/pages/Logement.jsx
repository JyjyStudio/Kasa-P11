		...
		<Container>
			...
			<GridContainer>
				<HouseInfosWrapper>
					...
					{ house.tags.map( tag => <Tag txt={tag} key={`${house.id}-${tag}`}/> ) }
				</HouseInfosWrapper>
				<HostInfosWrapper>
					<ProfilContainer>
						<HostName>{house.host.name}</HostName>
						<Img src={house.host.picture} borderRadius="50%" width="4rem" margin="0 0 0 .5rem" alt='host' />
					</ProfilContainer>
					<RatingContainer>
			...
			<DropdownContainer>
				<Dropdown txt="Description" width={viewportWidth > 760 ? "45%" : "100%"} content={house.description} />
				<Dropdown txt="Equipements" width={viewportWidth > 760 ? "45%" : "100%"} content={house.equipments} />
			</DropdownContainer>
		</Container>
	)
}
